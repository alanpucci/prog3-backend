...
class VentaHortalizaController extends Venta implements IApiUsable
{
    public function CargarUno($request, $response, $args)
    {
        try {
            $parametros = $request->getParsedBody();
            if(isset($parametros["cantidad"]) && isset($parametros["hortaliza"]) && isset($_FILES["archivo"]) && isset($parametros["mail"])){
                $venta = new Venta();
                $venta->cantidad = $parametros['cantidad'];
                $venta->hortaliza = $parametros['hortaliza'];
